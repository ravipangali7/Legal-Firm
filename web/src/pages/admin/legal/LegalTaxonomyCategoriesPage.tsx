...
import {
  deleteAdminActCategory,
  deleteAdminLegalCaseCategory,
  deleteAdminProcedureCategory,
  fetchAdminActCategories,
  fetchAdminLegalCaseCategories,
  fetchAdminProcedureCategories,
  patchAdminActCategory,
  patchAdminLegalCaseCategory,
  patchAdminProcedureCategory,
  postAdminActCategory,
  postAdminLegalCaseCategory,
  postAdminProcedureCategory,
  type LegalLibraryTaxonomyCategoryApi,
} from '@/lib/api';
import { proceduresListQueryKey } from '@/lib/proceduresListQuery';
import { slugify } from '@/lib/slugify';

export type LegalTaxonomyKind = 'act' | 'legal-case' | 'procedure';
...
type Config = {
  ...
  fetch: () => Promise<LegalLibraryTaxonomyCategoryApi[]>;
  post: (body: Partial<LegalLibraryTaxonomyCategoryApi>) => Promise<LegalLibraryTaxonomyCategoryApi>;
  patch: (id: string, body: Partial<LegalLibraryTaxonomyCategoryApi>) => Promise<LegalLibraryTaxonomyCategoryApi>;
  delete: (id: string) => Promise<void>;
  invalidate: (qc: ReturnType<typeof useQueryClient>) => void;
};

const CONFIG: Record<LegalTaxonomyKind, Config> = {
  act: {
    title: 'Act categories',
    deleteHint: 'Acts linked to this category may block deletion (database PROTECT).',
    queryKey: ['admin-act-categories'] as const,
    fetch: fetchAdminActCategories,
    post: postAdminActCategory,
    patch: patchAdminActCategory,
    delete: deleteAdminActCategory,
    invalidate: (qc) => {
  ...
  'legal-case': {
    ...
    deleteHint: 'Legal cases linked to this category may block deletion (database PROTECT).',
    queryKey: ['admin-legal-case-categories'] as const,
    fetch: fetchAdminLegalCaseCategories,
    post: postAdminLegalCaseCategory,
    patch: patchAdminLegalCaseCategory,
    delete: deleteAdminLegalCaseCategory,
    invalidate: (qc) => {
  ...
  procedure: {
    ...
    deleteHint: 'Procedures linked to this category may block deletion (database PROTECT).',
    queryKey: ['admin-procedure-categories'] as const,
    fetch: fetchAdminProcedureCategories,
    post: postAdminProcedureCategory,
    patch: patchAdminProcedureCategory,
    delete: deleteAdminProcedureCategory,
    invalidate: (qc) => {
...
export type LegalTaxonomyCategoriesPageProps = { kind: LegalTaxonomyKind };

const LegalTaxonomyCategoriesPage = ({ kind }: LegalTaxonomyCategoriesPageProps) => {
  ...
  const [viewRow, setViewRow] = useState<LegalLibraryTaxonomyCategoryApi | null>(null);
  const slugEditedManuallyRef = useRef(false);

  const invalidate = () => cfg.invalidate(qc);
  ...
  return (
    <div className="space-y-6">
      ...
      <RecordViewDialog
        ...
        title={viewRow ? `Category: ${viewRow.slug}` : 'Details'}
        value={viewRow}
      />
      ...
      <AlertDialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{cfg.deleteHint}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default LegalTaxonomyCategoriesPage;
